 ...
 *Lab4-Q1
 * SymbolGraph och DepthFirstPaths (DFS)
 */
import java.io.File;
...
{
    private final ST<String, Integer> st;// string -> index :Symboltabell där strängar är vertex namn och ints index
    private final String[] keys;// index  -> string: Array inverterat index, ger vertex namn associerat med varje int -index
    private static Graph G = null; // den underliggande grafen
    /*
     ...
     * läsa texten till index
     */
    public SymbolGraph(String filename, String delimiter) {
        st = new ST<String, Integer>(); //för att få varje vale få en index(Symboltabell)
        ...
        {
            String[] a = in.nextLine().split(delimiter);//genom att ansluta (split) läsa tomma plats i filen
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        StringBuilder s = new StringBuilder();//Skapa ett specificerad string(s)
        //Läsa alla Hörnen
        ...
        // Skapa en ny symbolgraf där ord separeras med "".  ta alla text till nummer
        SymbolGraph st = new SymbolGraph(lasaOut, " ");
        ...
        //Create a DFS object
        DFS.DepthFirstPaths search = new DFS.DepthFirstPaths(st.G(), vertex1);
        ...
        if (search.hasPathTo(st.index(filename)))//första Hörnen(v) input och titta på hela lista
        {
            ...
            for (int i : search.pathTo(st.index(delimiter), G))//andra Hörnen(v) och titta om det finns koppling
